...
{
    global $_config;    if ($is_signature)
    {
        global $lang_profile;        if (preg_match('%\[/?(?:quote|code|list|h)\b[^\]]*\]%i', $text))
    ...
    $text = preg_replace($pattern, $replace, $text);    $text = str_replace('*' . "\0" . ']', '*]', $text);    if ($_config['o_make_links'] == '1')
        $text = do_clickable($text);
    // If we split up the message before we have to concatenate it together again (code tags)
    if (isset($inside))
    {
        $outside = explode("\0", $text);
        $text = '';        $num_tokens = count($outside);        for ($i = 0; $i < $num_tokens; ++$i)
        {
            $text .= $outside[$i];
            if (isset($inside[$i]))
    ...
    if (empty($errors))
        $temp_text = preparse_tags($text, $errors, $is_signature);    if ($temp_text !== false)
        $text = $temp_text;
    // Remove empty tags
...
function preparse_tags($text, &$errors, $is_signature = false)
{
    ...
    $tags_nested = array('quote' => $_config['o_quote_depth'], 'list' => 5, '*' => 5);
    ...
    $tags_fix = array('quote', 'b', 'i', 'u', 'color', 'colour', 'url', 'email', 'h');    $split_text = preg_split("/(\[[\*a-zA-Z0-9-\/]*?(?:=.*?)?\])/", $text, - 1, PREG_SPLIT_DELIM_CAPTURE|PREG_SPLIT_NO_EMPTY);    $open_tags = array('post');
    $open_args = array('');
    $opened_tag = 0;
    $new_text = '';
    $current_ignore = '';
    $current_nest = '';
    $current_depth = array();
    $limit_bbcode = $tags;    foreach ($split_text as $current)
    {
        if ($current == '')
            continue;
        ...
        if (substr($current, 0, 1) != '[' || substr($current, - 1, 1) != ']')
        {
            ...
            if (in_array($open_tags[$opened_tag], $tags_inline) && strpos($current, "\n") !== false)
            {
                ...
                $current = '';                if (!_trim($split_current[0], "\n")) // the first part is a linebreak so we need to handle any open tags first
                    array_unshift($split_current, '');                for ($i = 1; $i < count($split_current); $i += 2)
                {
                    $temp_opened = array();
    ...
    if (in_array($temp_tag , $tags_inline))
                ...
                $new_text .= _trim($current);
            else
                $new_text .= $current;            continue;
        }
        ...
        }        if ($current_nest)
        {
            ...
            if ($current_tag != $current_nest)
                continue;            if (substr($current, 1, 1) == '/')
                $current_depth[$current_nest]--;
            else
                $current_depth[$current_nest]++;            if ($current_depth[$current_nest] <= $tags_nested[$current_nest])
                $current_nest = '';            continue;
        }
        ...
        while (!empty($temp_opened))
            ...
            }            if (in_array($current_tag, array_keys($tags_nested)))
            {
                ...
                $limit_bbcode = $tags;            $new_text .= $current;            continue;
        }
        else
        {
            ...
            if (in_array($current_tag, array_keys($tags_limit_bbcode)))
                ...
                $limit_bbcode = $tags;            if (in_array($current_tag, $tags_block) && !in_array($open_tags[$opened_tag], $tags_block) && $opened_tag != 0)
            {
            ...
            if (strpos($current, '=') !== false && in_array($current_tag, $tags_quotes))
            {
                ...
                $limit_bbcode = $tags_limit_bbcode[$current_tag];            $open_tags[] = $current_tag;
...
{
    global $lang_common;    if (strlen($type) != 1)
        $type = '*';    if (strpos($content, '[list') !== false)
    {
        $pattern = array('%\[list(?:=([1a*]))?+\]((?:(?:(?!\[list(?:=[1a*])?+\]|\[/list\]).)++|(?R))*+)\[/list\]%isxe');
        $replace = array('preparse_list_tag(\'$2\', \'$1\', $errors)');
        $content = preg_replace($pattern, $replace, $content);
    }    $items = explode('[*]', str_replace('\"', '"', $content));    $content = '';
    foreach ($items as $item)
    {
        if (_trim($item) != '')
            $content .= '[*' . "\0" . ']' . str_replace('[/*]', '', _trim($item)) . '[/*' . "\0" . ']' . "\n";
    }    return '[list=' . $type . ']' . "\n" . $content . '[/list]';
}
// Split text into chunks ($inside contains all text inside $start and $end, and $outside contains all text outside)
function split_text($text, $start, $end, &$errors, $retab = true)
{
    global $_config, $lang_common;    $tokens = explode($start, $text);    $outside[] = $tokens[0];    $num_tokens = count($tokens);
    for ($i = 1; $i < $num_tokens; ++$i)
    {
        $temp = explode($end, $tokens[$i]);        if (count($temp) != 2)
        {
        ...
        $inside[] = $temp[0];
        $outside[] = $temp[1];
    }    if ($_config['o_indent_num_spaces'] != 8 && $retab)
    {
        $spaces = str_repeat(' ', $_config['o_indent_num_spaces']);
...
function handle_url_tag($url, $link = '', $bbcode = false)
{
    ...
    // Ok, not very pretty :-)
    if (!$bbcode)
        $link = ($link == '' || $link == $url) ? ((utf8_strlen($url) > 55) ? utf8_substr($url, 0 , 39) . ' &#133; ' . utf8_substr($url, - 10) : $url) : stripslashes($link);    if ($bbcode)
    {
        if ($full_url == $link)
            return '[url]' . $link . '[/url]';
        else
        ...
        return _CHtml::link($link, $full_url);
}
// Turns an URL from the [img] tag into an <img> tag or a <a href...> tag
function handle_img_tag($url, $is_signature = false, $alt = null)
{
    global $lang_common, $_user;    if ($alt == null)
        $alt = $url;    $img_tag = _CHtml::link('&lt;' . $lang_common['Image link'] . '&gt;', $url);    if ($is_signature && $_user['show_img_sig'] != '0')
        $img_tag = '<img class="sigimage" src="' . $url . '" alt="' . _CHtml::encode($alt) . '" />';
    else if (!$is_signature && $_user['show_img'] != '0')
        $img_tag = '<span class="postimg"><img src="' . $url . '" alt="' . _CHtml::encode($alt) . '" /></span>';    return $img_tag;
}
...
function handle_list_tag($content, $type = '*')
{
        ...
        $type = '*';    if (strpos($content, '[list') !== false)
    {
        ...
        $content = preg_replace($pattern, $replace, $content);
    }    $content = preg_replace('#\s*\[\*\](.*?)\[/\*\]\s*#s', '<li><p>$1</p></li>', _trim($content));    if ($type == '*')
        $content = '<ul>' . $content . '</ul>';
    else
    if ($type == 'a')
        $content = '<ol class="alpha">' . $content . '</ol>';
    else
        $content = '<ol class="decimal">' . $content . '</ol>';    return '</p>' . $content . '<p>';
}
// Convert BBCodes to their HTML equivalent
function do_bbcode($text, $is_signature = false)
{
    global $lang_common, $_user, $_config;    if (strpos($text, '[quote') !== false)
    {
    ...
    $pattern[] = '#\[h\](.*?)\[/h\]#ms';    $replace[] = '<strong>$1</strong>';
    $replace[] = '<em>$1</em>';
    $replace[] = '<span class="bbu">$1</span>';
    $replace[] = '<span style="color: $1">$2</span>';
    $replace[] = '</p><h5>$1</h5><p>';    if (($is_signature && $_config['p_sig_img_tag'] == '1') || (!$is_signature && $_config['p_message_img_tag'] == '1'))
    {
    ...
    $pattern[] = '#\[url=([^\[]+?)\](.*?)\[/url\]#e';
    $pattern[] = '#\[email\]([^\[]*?)\[/email\]#';
    $pattern[] = '#\[email=([^\[]+?)\](.*?)\[/email\]#';    $replace[] = 'handle_url_tag(\'$1\')';
    $replace[] = 'handle_url_tag(\'$1\', \'$2\')';
    $replace[] = _CHtml::link('$1', 'mailto:$1');
    $replace[] = _CHtml::link('$2', 'mailto:$1');
    // This thing takes a while! :)
    $text = preg_replace($pattern, $replace, $text);    return $text;
}
// Make hyperlinks clickable
function do_clickable($text)
{
    $text = ' ' . $text;    $text = preg_replace('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(https?|ftp|news){1}://([\w\-]+\.([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-])?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img)\])#ie', 'stripslashes(\'$1$2$3$4\').handle_url_tag(\'$5://$6\', \'$5://$6\', true).stripslashes(\'$4$10$11$12\')', $text);
    $text = preg_replace('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(www|ftp)\.(([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-])?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img)\])#ie', 'stripslashes(\'$1$2$3$4\').handle_url_tag(\'$5.$6\', \'$5.$6\', true).stripslashes(\'$4$10$11$12\')', $text);    return substr($text, 1);
}
// Convert a series of smilies to images
function do_smilies($text)
{
    global $_config, $smilies;    $text = ' ' . $text . ' ';    foreach ($smilies as $smiley_text => $smiley_img)
    {
        if (strpos($text, $smiley_text) !== false)
            $text = preg_replace("#(?<=[>\s])" . preg_quote($smiley_text, '#') . "(?=\W)#m", '<img src="' . $_config['o_web_path'] . '/img/smilies/' . $smiley_img . '" width="15" height="15" alt="' . substr($smiley_img, 0, strrpos($smiley_img, '.')) . '" />', $text);
    }    return substr($text, 1, - 1);
}
// Parse message text
function parse_message($text, $hide_smilies)
{
    global $_config, $lang_common, $_user;    if ($_config['o_censoring'] == '1')
        $text = censor_words($text);
    // Convert applicable characters to HTML entities
    $text = _CHtml::encode($text);
    // If the message contains a code tag we have to split it up (text within [code][/code] shouldn't be touched)
    if (strpos($text, '[code]') !== false && strpos($text, '[/code]') !== false)
    {
        list($inside, $outside) = split_text($text, '[code]', '[/code]', $errors);
        $text = implode("\0", $outside);
    }    if ($_config['p_message_bbcode'] == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
        $text = do_bbcode($text);    if ($_config['o_smilies'] == '1' && $_user['show_smilies'] == '1' && $hide_smilies == '0')
    ...
    if (isset($inside))
    {
        ...
        $text = '';        $num_tokens = count($outside);        for ($i = 0; $i < $num_tokens; ++$i)
        {
            $text .= $outside[$i];
            if (isset($inside[$i]))
                // $text .= '</p><div class="codebox"><pre><code>'._trim($inside[$i], "\n\r").'</code></pre></div><p>';
                {
                    $num_lines = (substr_count($inside[$i], "\n"));
                $text .= '</p><div class="codebox"><pre' . (($num_lines > 28) ? ' class="vscroll"' : '') . '><code>' . _trim($inside[$i], "\n\r") . '</code></pre></div><p>';
            }
        }
    }
    ...
    $text = str_replace('<p></p>', '', '<p>' . $text . '</p>');    return $text;
}
...
function parse_signature($text)
{
    global $_config, $lang_common, $_user;    if ($_config['o_censoring'] == '1')
        $text = censor_words($text);
    // Convert applicable characters to HTML entities
    $text = _CHtml::encode($text);    if ($_config['p_sig_bbcode'] == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
        $text = do_bbcode($text, true);    if ($_config['o_smilies_sig'] == '1' && $_user['show_smilies'] == '1')
        $text = do_smilies($text);
    // Deal with newlines, tabs and multiple spaces
    $pattern = array("\n", "\t", '  ', '  ');
    $replace = array('<br />', '&nbsp; &nbsp; ', '&nbsp; ', ' &nbsp;');
