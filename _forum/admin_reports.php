...
{
    confirm_referrer('admin_reports.php');    $zap_id = intval(key($_POST['zap_id']));    $db->setQuery('SELECT zapped FROM forum_reports WHERE id=' . $zap_id) or error('Unable to fetch report info', __FILE__, __LINE__, $db->error());
    $zapped = $db->result();    if ($zapped == '')
        $db->setQuery('UPDATE forum_reports SET zapped=' . time() . ', zapped_by=' . $_user['id'] . ' WHERE id=' . $zap_id)->execute() or error('Unable to zap report', __FILE__, __LINE__, $db->error());    redirect('admin_reports.php', 'Report zapped. Redirecting &hellip;');
