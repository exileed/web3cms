...
// Delete a category
    ...
    if (isset($_POST['del_cat_comply'])) { // Delete a category with all forums and posts
            @set_time_limit(0);
        $db->setQuery('SELECT id FROM forum_forums WHERE cat_id=' . $cat_to_delete) or error('Unable to fetch forum list', __FILE__, __LINE__, $db->error());
...
<?php require SHELL_PATH . 'footer.php';
    ...
    $db->setQuery('SELECT id, disp_position FROM forum_categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
    ...
    for ($i = 0; $i < $num_cats; ++$i) {
        ...
        $db->setQuery('UPDATE forum_categories SET cat_name=\'' . $db->escape($cat_name[$i]) . '\', disp_position=' . $cat_order[$i] . ' WHERE id=' . $cat_id)->execute() or error('Unable to update category', __FILE__, __LINE__, $db->error());
    }
    // Regenerate the quick jump cache
    if (!defined('FORUM_CACHE_FUNCTIONS_LOADED'))
        require SHELL_PATH . 'include/cache.php';
