...
function check_cookie(&$_user)
{
    ...
    if ($cookie['user_id'] > 1) {
        ...
        $db->setQuery('SELECT u.*, g.*, o.logged, o.idle FROM w3_user AS u INNER JOIN w3_user_details AS ud INNER JOIN forum_groups AS g ON ud.forumGroupId=g.g_id LEFT JOIN forum_online AS o ON o.user_id=u.id WHERE u.id=' . intval($cookie['user_id'])) or error('Unable to fetch user information', __FILE__, __LINE__, $db->error());
        ...
        if (!defined('PUN_QUIET_VISIT')) {
            ...
            if (!$_user['logged']) {
                ...
                if ($_user['logged'] < ($now - $_config['o_timeout_visit'])) {
                    $db->setQuery('UPDATE w3_user SET last_visit=' . $_user['logged'] . ' WHERE id=' . $_user['id'])->execute() or error('Unable to update user visit data', __FILE__, __LINE__, $db->error());
...
function authenticate_user($user, $password, $password_is_hash = false)
{
    ...
    $db->setQuery('SELECT u.*, g.*, o.logged, o.idle FROM w3_user AS u INNER JOIN w3_user_details AS ud INNER JOIN forum_groups AS g ON g.g_id=ud.forumGroupId LEFT JOIN forum_online AS o ON o.user_id=u.id WHERE ' . (is_int($user) ? 'u.id=' . intval($user) : 'u.username=\'' . $db->escape($user) . '\'')) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
    $_user = $db->fetch_assoc();
    if (!isset($_user['id']) ||
...
function set_default_user()
{
    ...
    // Fetch guest user
    $db->setQuery('SELECT u.*, g.*, o.logged, o.last_post, o.last_search FROM w3_user AS u INNER JOIN w3_user_details AS ud INNER JOIN forum_groups AS g ON ud.forumGroupId=g.g_id LEFT JOIN forum_online AS o ON o.ident=\'' . $remote_addr . '\' WHERE u.id=1') or error('Unable to fetch guest information', __FILE__, __LINE__, $db->error());
    if (!$db->num_rows())
        exit('Unable to fetch guest information. The table \'w3_user\' must contain an entry with id = 1 that represents anonymous users.');
    $_user = $db->fetch_assoc();
    // Update online list
    if (!$_user['logged']) {
        $_user['logged'] = time();
        ...
        $db->setQuery('UPDATE forum_online SET logged=' . time() . ' WHERE ident=\'' . $db->escape($remote_addr) . '\'')->execute() or error('Unable to update online list', __FILE__, __LINE__, $db->error());
    $_user['disp_topics'] = $_config['o_disp_topics_default'];
    $_user['disp_posts'] = $_config['o_disp_posts_default'];
    ...
    while ($cur_user = $db->fetch_assoc()) {
        ...
        if ($cur_user['user_id'] == '1')
            ...
            if ($cur_user['logged'] < ($now - $_config['o_timeout_visit'])) {
                $db->setQuery('UPDATE w3_user SET last_visit=' . $cur_user['logged'] . ' WHERE id=' . $cur_user['user_id'])->execute() or error('Unable to update user visit data', __FILE__, __LINE__, $db->error());
...
function generate_navlinks()
{
    ...
    $links[] = '<li id="navindex">' . _CHtml::link($lang_common['Index'], array('forum/'));
    if ($_user['g_read_board'] == '1' && $_user['g_view_users'] == '1')
...
<div id="profile" class="block2col">
    ...
    echo _CHtml::link($lang_profile['Section essentials'], array('forum/profile', 'section' => 'essentials', 'id' => $id));
    ?></li>
    ...
    echo _CHtml::link($lang_profile['Section messaging'], array('forum/profile', 'section' => 'messaging', 'id' => $id));
    ?></li>
<?php if ($_config['o_avatars'] == '1' || $_config['o_signatures'] == '1'): ?>					<li<?php if ($page == 'personality') echo ' class="isactive"';?>><?php
        echo _CHtml::link($lang_profile['Section personality'], array('forum/profile', 'section' => 'personality', 'id' => $id));
        ?></li>
<?php endif; ?>					<li<?php if ($page == 'display') echo ' class="isactive"';?>><?php
        echo _CHtml::link($lang_profile['Section display'], array('forum/profile', 'section' => 'display', 'id' => $id));
        ?></li>
