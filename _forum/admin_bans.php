...
require SHELL_PATH . 'include/common_admin.php';if ($_user['g_id'] != PUN_ADMIN && ($_user['g_moderator'] != '1' || $_user['g_mod_ban_users'] == '0'))
...
if (isset($_REQUEST['add_ban']) || isset($_GET['edit_ban']))
{
    if (isset($_GET['add_ban']) || isset($_POST['add_ban']))
    {
        ...
        if (isset($_GET['add_ban']))
        {
            ...
            if ($add_ban < 2)
                message($lang_common['Bad request']);            $user_id = $add_ban;            $db->setQuery('SELECT group_id, username, email FROM forum_userprofiles WHERE id=' . $user_id) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
            if ($db->num_rows())
                list($group_id, $ban_user, $ban_email) = $db->fetch_row();
            else
                message('No user by that ID registered.');
        }
        else // Otherwise the username is in POST
            {
                $ban_user = trim($_POST['new_ban_user']);            if ($ban_user != '')
            {
                $db->setQuery('SELECT id, group_id, username, email FROM forum_userprofiles WHERE username=\'' . $db->escape($ban_user) . '\' AND id>1') or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
                if ($db->num_rows())
                    list($user_id, $group_id, $ban_user, $ban_email) = $db->fetch_row();
                else
        ...
        if ($ban_id < 1)
            message($lang_common['Bad request']);        $db->setQuery('SELECT username, ip, email, message, expire FROM forum_bans WHERE id=' . $ban_id) or error('Unable to fetch ban info', __FILE__, __LINE__, $db->error());
        if ($db->num_rows())
            list($ban_user, $ban_ip, $ban_email, $ban_message, $ban_expire) = $db->fetch_row();
        else
            message($lang_common['Bad request']);        $ban_expire = ($ban_expire != '') ? date('Y-m-d', $ban_expire) : '';        $mode = 'edit';
    }    $focus_element = array('bans2', 'ban_user');
    require SHELL_PATH . 'header.php';    generate_admin_menu('bans');    ?>
	<div class="blockform">
...
{
    confirm_referrer('admin_bans.php');    $ban_user = trim($_POST['ban_user']);
    $ban_ip = trim($_POST['ban_ip']);
    $ban_email = strtolower(trim($_POST['ban_email']));
    $ban_message = trim($_POST['ban_message']);
    $ban_expire = trim($_POST['ban_expire']);    if ($ban_user == '' && $ban_ip == '' && $ban_email == '')
    ...
    if ($ban_ip != '')
    {
        ...
        $addresses = array_map('_trim', $addresses);        for ($i = 0; $i < count($addresses); ++$i)
        {
            if (strpos($addresses[$i], ':') !== false)
            {
                $octets = explode(':', $addresses[$i]);                for ($c = 0; $c < count($octets); ++$c)
                {
...
if ($c > 3 || preg_match('/[^0-9]/', $octets[$c]) || intval($octets[$c]) > 255)
    ...
    {
        $ban_expire = strtotime($ban_expire);        if ($ban_expire == - 1 || $ban_expire <= time())
        ...
        $ban_expire = 'NULL';    $ban_user = ($ban_user != '') ? '\'' . $db->escape($ban_user) . '\'' : 'NULL';
    $ban_ip = ($ban_ip != '') ? '\'' . $db->escape($ban_ip) . '\'' : 'NULL';
    $ban_email = ($ban_email != '') ? '\'' . $db->escape($ban_email) . '\'' : 'NULL';
    $ban_message = ($ban_message != '') ? '\'' . $db->escape($ban_message) . '\'' : 'NULL';    if ($_POST['mode'] == 'add')
    ...
    if (!defined('FORUM_CACHE_FUNCTIONS_LOADED'))
        require SHELL_PATH . 'include/cache.php';    generate_bans_cache();    redirect('admin_bans.php', 'Ban ' . (($_POST['mode'] == 'edit') ? 'edited' : 'added') . '. Redirecting &hellip;');
}
// Remove a ban
else if (isset($_GET['del_ban']))
{
    confirm_referrer('admin_bans.php');    $ban_id = intval($_GET['del_ban']);
    if ($ban_id < 1)
        message($lang_common['Bad request']);    $db->setQuery('DELETE FROM forum_bans WHERE id=' . $ban_id)->execute() or error('Unable to delete ban', __FILE__, __LINE__, $db->error());
    // Regenerate the bans cache
    if (!defined('FORUM_CACHE_FUNCTIONS_LOADED'))
        require SHELL_PATH . 'include/cache.php';    generate_bans_cache();    redirect('admin_bans.php', 'Ban removed. Redirecting &hellip;');
...
require SHELL_PATH . 'header.php';generate_admin_menu('bans');?>
	<div class="blockform">
		...
		<div class="box">
			<div class="fakeform">
<?php $db->setQuery('SELECT b.id, b.username, b.ip, b.email, b.message, b.expire, b.ban_creator, u.username AS ban_creator_username FROM forum_bans AS b LEFT JOIN forum_userprofiles AS u ON b.ban_creator=u.id ORDER BY b.id') or error('Unable to fetch ban list', __FILE__, __LINE__, $db->error());
if ($db->num_rows())
{
