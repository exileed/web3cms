...
require SHELL_PATH . 'include/common.php';if ($_user['g_read_board'] == '0')
    message($lang_common['No view']);$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
if ($id < 1)
...
if ($cur_forum['moderators'] != '')
    $mods_array = unserialize($cur_forum['moderators']);$is_admmod = ($_user['g_id'] == PUN_ADMIN || ($_user['g_moderator'] == '1' && array_key_exists($_user['username'], $mods_array))) ? true : false;
...
$paging_links = $lang_common['Pages'] . ': ' . paginate($num_pages, $p, 'viewforum.php?id=' . $id);$page_title = _CHtml::encode($this->PageTitle . ' / ' . $cur_forum['forum_name']);
...
if ($db->num_rows())
{
    while ($cur_topic = $db->fetch_assoc())
    {
        ...
        $icon_type = 'icon';        if ($cur_topic['moved_to'] == null)
            ...
            $last_post = '&nbsp;';        if ($_config['o_censoring'] == '1')
