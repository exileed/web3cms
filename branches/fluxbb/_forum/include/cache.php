...
if (!function_exists('var_export'))
{
    ...
    {
        $args = func_get_args();
        $indent = (isset($args[2])) ? $args[2] : '';        if (is_array($args[0]))
        {
            $output = 'array (' . "\n";            foreach ($args[0] as $k => $v)
            {
                if (is_numeric($k))
                    ...
                    $output .= $indent . '  \'' . str_replace('\'', '\\\'', str_replace('\\', '\\\\', $k)) . '\' => ';                if (is_array($v))
...
function generate_bans_cache()
{
    ...
    $db->setQuery('SELECT * FROM forum_bans', true) or error('Unable to fetch ban list', __FILE__, __LINE__, $db->error());    $output = array();
    while ($cur_ban = $db->fetch_assoc())
    $output[] = $cur_ban;
    // Output ban list as PHP code
    $fh = @fopen(FORUM_CACHE_DIR . 'cache_bans.php', 'wb');
    if (!$fh)
        error('Unable to write bans cache file to cache directory. Please make sure PHP has write access to the directory \'cache\'', __FILE__, __LINE__);    fwrite($fh, '<?php' . "\n\n" . 'define(\'PUN_BANS_LOADED\', 1);' . "\n\n" . '$_bans = ' . var_export($output, true) . ';' . "\n\n" . '?>');    fclose($fh);
}
...
function generate_ranks_cache()
{
    ...
    $db->setQuery('SELECT * FROM forum_ranks ORDER BY min_posts', true) or error('Unable to fetch rank list', __FILE__, __LINE__, $db->error());    $output = array();
    while ($cur_rank = $db->fetch_assoc())
    $output[] = $cur_rank;
    // Output ranks list as PHP code
    $fh = @fopen(FORUM_CACHE_DIR . 'cache_ranks.php', 'wb');
    if (!$fh)
        error('Unable to write ranks cache file to cache directory. Please make sure PHP has write access to the directory \'cache\'', __FILE__, __LINE__);    fwrite($fh, '<?php' . "\n\n" . 'define(\'PUN_RANKS_LOADED\', 1);' . "\n\n" . '$_ranks = ' . var_export($output, true) . ';' . "\n\n" . '?>');    fclose($fh);
}
...
function generate_quickjump_cache($group_id = false)
{
    ...
    else
    {
        ...
        $num_groups = $db->num_rows();        for ($i = 0; $i < $num_groups; ++$i)
    ...
    while (list(, $group_id) = @each($groups))
    {
        ...
        if (!$fh)
            error('Unable to write quick jump cache file to cache directory. Please make sure PHP has write access to the directory \'cache\'', __FILE__, __LINE__);        $output = '<?php' . "\n\n" . 'if (!defined(\'PUN\')) exit;' . "\n" . 'define(\'PUN_QJ_LOADED\', 1);' . "\n\n" . '?>';
        $output .= "\t\t\t\t" . _CHtml::form('forum/viewforum', 'GET',array('id'=>'qjump')) . "\n\t\t\t\t\t" . '<div><label><?php echo $lang_common[\'Jump to\'] ?>' . "\n\n\t\t\t\t\t" . '<br /><select name="id" onchange="window.location=(\''.Yii::app()->createUrl('forum/viewforum',array('id'=>'+this.options[this.selectedIndex].value')).'\')">' . "\n";        $db->setQuery('SELECT c.id AS cid, c.cat_name, f.id AS fid, f.forum_name, f.redirect_url FROM forum_categories AS c INNER JOIN forum_forums AS f ON c.id=f.cat_id LEFT JOIN forum_forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id=' . $group_id . ') WHERE fp.read_forum IS NULL OR fp.read_forum=1 ORDER BY c.disp_position, c.id, f.disp_position', true) or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());        $cur_category = 0;
