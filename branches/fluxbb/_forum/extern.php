...
/*---	Copyright (C) 2008-2009 FluxBB.org
  ...
  mandatory variable is action. Possible/default values are:    action: feed - show most recent topics/posts (HTML or RSS)
            ...
            posted - show topics ordered by when they were first
                     posted, giving information about the original post.-----------------------------------------------------------------------------*/define('PUN_QUIET_VISIT', 1);if (!defined('SHELL_PATH'))    require SHELL_PATH . 'include/common.php';
...
if ($_user['is_guest'] && isset($_SERVER['PHP_AUTH_USER']))
    authenticate_user($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']);if ($_user['g_read_board'] == '0')
{
...
{
    global $_config, $_user;    if (!$_user['is_guest'])
        return;    header('WWW-Authenticate: Basic realm="Web3CMS External Syndication"');
...
function output_rss($feed)
{
    ...
    header('Pragma: public');    echo '<?xml version="1.0" encoding="utf-8"?>' . "\n";
...
function output_atom($feed)
{
    ...
    echo "\t" . '<updated>' . gmdate('Y-m-d\TH:i:s\Z', count($feed['items']) ? $feed['items'][0]['pubdate'] : time()) . '</updated>' . "\n";    if ($_config['o_show_version'] == '1')
        ...
        echo "\t" . '<generator>FluxBB</generator>' . "\n";    echo "\t" . '<id>' . $feed['link'] . '</id>' . "\n";    $content_tag = ($feed['type'] == 'posts') ? 'content' : 'summary';    foreach ($feed['items'] as $item)
    {
        echo "\t\t" . '<entry>' . "\n";
...
function output_xml($feed)
{
    ...
    header('Pragma: public');    echo '<?xml version="1.0" encoding="utf-8"?>' . "\n";
    echo '<source>' . "\n";
    echo "\t" . '<url>' . $feed['link'] . '</url>' . "\n";    $forum_tag = ($feed['type'] == 'posts') ? 'post' : 'topic';    foreach ($feed['items'] as $item)
    {
...
function output_html($feed)
{
    ...
    header('Expires: ' . gmdate('D, d M Y H:i:s') . ' GMT');
    header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
    header('Pragma: public');    foreach ($feed['items'] as $item)
    {
        if (utf8_strlen($item['title']) > FORUM_EXTERN_MAX_SUBJECT_LENGTH)
            ...
            $subject_truncated = _CHtml::encode($item['title']);        echo '<li>' . _CHtml::link($subject_truncated, array('forum/' . $item['link']), array('title' => _CHtml::encode($item['title']))) . '</li>' . "\n";
    }
}
...
if ($action == 'feed')
{
    ...
    $type = isset($_GET['type']) && in_array($_GET['type'], array('html', 'rss', 'atom', 'xml')) ? $_GET['type'] : 'html';    $show = isset($_GET['show']) ? intval($_GET['show']) : 15;
    if ($show < 1 || $show > 50)
        $show = 15;
    ...
    if (isset($_GET['tid']))
    {
        ...
        }        $cur_topic = $db->fetch_assoc();        if ($_config['o_censoring'] == '1')
        ...
        // Fetch $show posts
        $db->setQuery('SELECT p.id, p.poster, p.message, p.hide_smilies, p.posted, p.poster_id, u.email_setting, u.email, p.poster_email FROM forum_posts AS p INNER JOIN forum_userprofiles AS u ON u.id=p.poster_id WHERE p.topic_id=' . $tid . ' ORDER BY p.posted DESC LIMIT ' . $show) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
        while ($cur_post = $db->fetch_assoc())
        {
            $cur_post['message'] = parse_message($cur_post['message'], $cur_post['hide_smilies']);            $item = array(
                'id' => $cur_post['id'],
                ...
                );            if ($cur_post['poster_id'] > 1)
            {
                if ($cur_post['email_setting'] == '0' && !$_user['is_guest'])
                    $item['author']['email'] = $cur_post['email'];                $item['author']['uri'] = $_config['o_web_path'] . '/profile.php?id=' . $cur_post['poster_id'];
            }
            else if ($cur_post['poster_email'] != '' && !$_user['is_guest'])
                $item['author']['email'] = $cur_post['poster_email'];            $feed['items'][] = $item;
    ...
    else
    {
        ...
        if (isset($_GET['fid']) && is_scalar($_GET['fid']) && $_GET['fid'] != '')
        {
            ...
            $fids = array_map('intval', $fids);            if (!empty($fids))
                $forum_sql = ' AND t.forum_id IN(' . implode(',', $fids) . ')';            if (count($fids) == 1)
            {
                // Fetch forum name
        ...
        if (isset($_GET['nfid']) && is_scalar($_GET['nfid']) && $_GET['nfid'] != '')
        {
            ...
            $nfids = array_map('intval', $nfids);            if (!empty($nfids))
        ...
        // Fetch $show topics
        $db->setQuery('SELECT t.id, t.poster, t.subject, t.posted, t.last_post, t.last_poster, p.message, p.hide_smilies, u.email_setting, u.email, p.poster_id, p.poster_email FROM forum_topics AS t INNER JOIN forum_posts AS p ON p.id=' . ($order_posted ? 't.first_post_id' : 't.last_post_id') . ' INNER JOIN forum_userprofiles AS u ON u.id=p.poster_id LEFT JOIN forum_forum_perms AS fp ON (fp.forum_id=t.forum_id AND fp.group_id=' . $_user['g_id'] . ') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.moved_to IS NULL' . $forum_sql . ' ORDER BY ' . ($order_posted ? 't.posted' : 't.last_post') . ' DESC LIMIT ' . $show) or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
        while ($cur_topic = $db->fetch_assoc())
        {
            $cur_topic['message'] = parse_message($cur_topic['message'], $cur_topic['hide_smilies']);            if ($_config['o_censoring'] == '1')
                $cur_topic['subject'] = censor_words($cur_topic['subject']);            $item = array(
                'id' => $cur_topic['id'],
                ...
                );            if ($cur_topic['poster_id'] > 1)
            {
                if ($cur_topic['email_setting'] == '0' && !$_user['is_guest'])
                    $item['author']['email'] = $cur_topic['email'];                $item['author']['uri'] = $_config['o_web_path'] . '/profile.php?id=' . $cur_topic['poster_id'];
            }
            else if ($cur_topic['poster_email'] != '' && !$_user['is_guest'])
                $item['author']['email'] = $cur_topic['poster_email'];            $feed['items'][] = $item;
        }        $output_func = 'output_' . $type;
        $output_func($feed);
    }    exit;
}
...
else if ($action == 'online' || $action == 'online_full')
{
    ...
    $users = array();    $db->setQuery('SELECT user_id, ident FROM forum_online WHERE idle=0 ORDER BY ident', true) or error('Unable to fetch online list', __FILE__, __LINE__, $db->error());    while ($_user_online = $db->fetch_assoc())
    {
    ...
    header('Expires: ' . gmdate('D, d M Y H:i:s') . ' GMT');
    header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
    header('Pragma: public');    echo $lang_index['Guests online'] . ': ' . forum_number_format($num_guests) . '<br />' . "\n";    if ($action == 'online_full' && !empty($users))
...
else if ($action == 'stats')
{
    ...
    $db->setQuery('SELECT COUNT(id)-1 FROM forum_userprofiles') or error('Unable to fetch total user count', __FILE__, __LINE__, $db->error());
    $stats['total_users'] = $db->result();    $db->setQuery('SELECT id, username FROM forum_userprofiles ORDER BY registered DESC LIMIT 1') or error('Unable to fetch newest registered user', __FILE__, __LINE__, $db->error());
    ...
    header('Expires: ' . gmdate('D, d M Y H:i:s') . ' GMT');
    header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
    header('Pragma: public');    echo $lang_index['No of users'] . ': ' . forum_number_format($stats['total_users']) . '<br />' . "\n";
