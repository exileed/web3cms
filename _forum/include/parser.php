...
function handle_url_tag($url, $link = '', $bbcode = false)
{
    ...
    if ($bbcode)
    {
        ...
        return CHtml::link($link, $full_url);
}
...
function handle_img_tag($url, $is_signature = false, $alt = null)
{
    ...
    $img_tag = CHtml::link('&lt;' . $lang_common['Image link'] . '&gt;', $url);
...
function do_bbcode($text, $is_signature = false)
{
    ...
    $replace[] = CHtml::link('$1', 'mailto:$1');
    $replace[] = CHtml::link('$2', 'mailto:$1');
...
function do_smilies($text)
{
    ...
    foreach ($smilies as $smiley_text => $smiley_img)
    {
        if (strpos($text, $smiley_text) !== false)
            $text = preg_replace("#(?<=[>\s])" . preg_quote($smiley_text, '#') . "(?=\W)#m", '<img src="' . $pun_config['o_WEB_PATH'] . '/img/smilies/' . $smiley_img . '" width="15" height="15" alt="' . substr($smiley_img, 0, strrpos($smiley_img, '.')) . '" />', $text);
    }
