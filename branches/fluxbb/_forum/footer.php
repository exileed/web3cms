...
if (!defined('PUN'))
    exit;$tpl_temp = trim(ob_get_contents());
...
$footer_style = isset($footer_style) ? $footer_style : null;if ($footer_style == 'index' || $footer_style == 'search')
{
...
else if ($footer_style == 'viewforum' || $footer_style == 'viewtopic')
{
    ...
    if ($_config['o_quickjump'] == '1')
    {
        ...
        if (file_exists(FORUM_CACHE_DIR . 'cache_quickjump_' . $_user['g_id'] . '.php'))
            include FORUM_CACHE_DIR . 'cache_quickjump_' . $_user['g_id'] . '.php';        if (!defined('PUN_QJ_LOADED'))
        {
            if (!defined('FORUM_CACHE_FUNCTIONS_LOADED'))
                require SHELL_PATH . 'include/cache.php';            generate_quickjump_cache($_user['g_id']);
...
if (defined('PUN_SHOW_QUERIES'))
    display_saved_queries();$tpl_temp = trim(ob_get_contents());
