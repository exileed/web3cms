...
if ($cur_topic['closed'] == '0')
{
    if (($cur_topic['post_replies'] == '' && $pun_user['g_post_replies'] == '1') || $cur_topic['post_replies'] == '1' || $is_admmod)
        $post_link = CHtml::link($lang_topic['Post reply'], array('forum/post', 'tid' => $id));
    else
        $post_link = '&nbsp;';
}
else
...
if (!$pun_user['is_guest'] && $pun_config['o_subscriptions'] == '1')
{
    if ($cur_topic['is_subscribed'])
        ...
        $subscraction = '<p class="subscribelink clearb"><span>' . $lang_topic['Is subscribed'] . ' - </span>' . CHtml::link($lang_topic['Unsubscribe'], array('forum/misc', 'unsubscribe' => $id)) . '</p>' . "\n";
    else
        $subscraction = '<p class="subscribelink clearb">' . CHtml::link($lang_topic['Subscribe'], array('forum/misc', 'subscribe' => $id)) . '</p>' . "\n";
}
else
    $subscraction = '';
...
<div class="linkst">
	<div class="inbox">
		<ul class="crumbs">
			...
			<li>&raquo;&nbsp;
			<?php echo CHtml::link(pun_htmlspecialchars($cur_topic['forum_name']), array('forum/viewforum', 'id' => $cur_topic['forum_id']));?></li>
...
while ($cur_post = $db->fetch_assoc())
{
    ...
    if ($cur_post['poster_id'] > 1)
    {
        if ($pun_user['g_view_users'] == '1')
            $username = CHtml::link(pun_htmlspecialchars($cur_post['username']), array('forum/profile', 'id' => $cur_post['poster_id']));
        else
        ...
        if ($pun_config['o_show_user_info'] == '1')
        {
            ...
            if ((($cur_post['email_setting'] == '0' && !$pun_user['is_guest']) || $pun_user['is_admmod']) && $pun_user['g_send_email'] == '1')
                $user_contacts[] = CHtml::link($lang_common['Email'], 'mailto:' . $cur_post['email']);
            else if ($cur_post['email_setting'] == '1' && !$pun_user['is_guest'] && $pun_user['g_send_email'] == '1')
                $user_contacts[] = CHtml::link($lang_common['Email'], array('forum/misc', 'email' => $cur_post['poster_id']));

            if ($cur_post['url'] != '')
                $user_contacts[] = CHtml::link($lang_topic['Website'], pun_htmlspecialchars($cur_post['url']));
        }

        if ($pun_user['is_admmod'])
        {
            $user_info[] = '<dd>' . $lang_topic['IP'] . ': ' . CHtml::link($cur_post['poster_ip'], array('forum/moderate', 'get_host' => $cur_post['id']));
    ...
    else
    {
        ...
        if ($pun_config['o_show_user_info'] == '1' && $cur_post['poster_email'] != '' && !$pun_user['is_guest'] && $pun_user['g_send_email'] == '1')
            $user_contacts[] = CHtml::link($lang_common['Email'], 'mailto:' . $cur_post['poster_email']);
    }
    // Generation post action array (quote, edit, delete etc.)
    if (!$is_admmod)
    {
        if (!$pun_user['is_guest'])
            $post_actions[] = '<li class="postreport">' . CHtml::link($lang_topic['Report'], array('forum/misc', 'report' => $cur_post['id']));
        ...
        if ($cur_topic['closed'] == '0')
        {
            if ($cur_post['poster_id'] == $pun_user['id'])
            {
                if ((($start_from + $post_count) == 1 && $pun_user['g_delete_topics'] == '1') || (($start_from + $post_count) > 1 && $pun_user['g_delete_posts'] == '1'))
                    $post_actions[] = '<li class="postdelete">' . CHtml::link($lang_topic['Delete'], array('forum/delete', 'id' => $cur_post['id']));
                if ($pun_user['g_edit_posts'] == '1')
                    $post_actions[] = '<li class="postedit">' . CHtml::link($lang_topic['Edit'], array('forum/edit', 'id' => $cur_post['id']));
            }

            if (($cur_topic['post_replies'] == '' && $pun_user['g_post_replies'] == '1') || $cur_topic['post_replies'] == '1')
                $post_actions[] = '<li class="postquote">' . CHtml::link($lang_topic['Quote'], array('forum/post', 'tid' => $id, 'qid' => $cur_post['id']));
        }
    }
    else
        $post_actions[] = '<li class="postreport">' . CHtml::link($lang_topic['Report'], array('forum/misc', 'report' => $cur_post['id'])) . $lang_common['Link separator'] . '</li><li class="postdelete">' . CHtml::link($lang_topic['Delete'], array('forum/delete', 'id' => $cur_post['id'])) . $lang_common['Link separator'] . '</li><li class="postedit">' . CHtml::link($lang_topic['Edit'], array('forum/edit', 'id' => $cur_post['id'])) . $lang_common['Link separator'] . '</li><li class="postquote">' . CHtml::link($lang_topic['Quote'], array('forum/post', 'tid' => $id, 'qid' => $cur_post['id']));
...
<div id="p<?php echo $cur_post['id'] ?>" class="blockpost<?php echo ($post_count % 2 == 0) ? ' roweven' : ' rowodd' ?><?php if (($post_count + $start_from) == 1) echo ' firstpost'; ?>">
	...
	<?php echo CHtml::link(format_time($cur_post['posted']), array('forum/viewtopic', 'pid' => $cur_post['id'] . '#p' . $cur_post['id']));?></a></span></h2>
	<div class="box">
		<div class="inbox">
			<div class="postbody">
				<div class="postleft">
...
<div id="quickpost" class="blockform">
	...
	<div class="box">
		<?php echo CHtml::form(array('post','tid'=>$id ), 'POST', array('onsubmit'=>'this.submit.disabled=true;if(process_form(this)){return true;}else{this.submit.disabled=false;return false;}'));?>
			<div class="inform">
				<fieldset>
