...
if ($_user['g_read_board'] == '0')
    message($lang_common['No view']);$action = isset($_GET['action']) ? $_GET['action'] : null;
...
if ($pid)
{
    ...
    if (!$db->num_rows())
        message($lang_common['Bad request']);    $id = $db->result();
    ...
    $num_posts = $db->num_rows();    for ($i = 0; $i < $num_posts; ++$i)
    {
...
else if ($action == 'new' && !$_user['is_guest'])
{
    ...
    $last_viewed = isset($tracked_topics['topics'][$id]) ? $tracked_topics['topics'][$id] : $_user['last_visit'];    $db->setQuery('SELECT MIN(id) FROM forum_posts WHERE topic_id=' . $id . ' AND posted>' . $last_viewed) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
    $first_new_post_id = $db->result();    if ($first_new_post_id)
...
else if ($action == 'last')
{
    ...
    $last_post_id = $db->result();    if ($last_post_id)
    {
...
$paging_links = $lang_common['Pages'] . ': ' . paginate($num_pages, $p, 'viewtopic.php?id=' . $id);if ($_config['o_censoring'] == '1')
    $cur_topic['subject'] = censor_words($cur_topic['subject']);$quickpost = false;
...
while ($cur_post = $db->fetch_assoc())
{
    ...
    if ($cur_post['poster_id'] > 1)
    {
        if ($_user['g_view_users'] == '1')
            ...
            $username = _CHtml::encode($cur_post['username']);        $user_title = get_title($cur_post);        if ($_config['o_censoring'] == '1')
            $user_title = censor_words($user_title);
        // Format the online indicator
        $is_online = ($cur_post['is_online'] == $cur_post['poster_id']) ? '<strong>' . $lang_topic['Online'] . '</strong>' : $lang_topic['Offline'];        if ($_config['o_avatars'] == '1' && $_user['show_avatars'] != '0')
        {
        ...
        if ($_config['o_show_user_info'] == '1')
        {
            if ($cur_post['location'] != '')
            {
                if ($_config['o_censoring'] == '1')
                    $cur_post['location'] = censor_words($cur_post['location']);                $user_info[] = '<dd>' . $lang_topic['From'] . ': ' . _CHtml::encode($cur_post['location']);
            }            $user_info[] = '<dd>' . $lang_common['Registered'] . ': ' . MDate::format($cur_post['registered'], true);            if ($_config['o_show_post_count'] == '1' || $_user['is_admmod'])
            ...
            if ((($cur_post['email_setting'] == '0' && !$_user['is_guest']) || $_user['is_admmod']) && $_user['g_send_email'] == '1')
                ...
                $user_contacts[] = _CHtml::link($lang_common['Email'], array('forum/misc', 'email' => $cur_post['poster_id']));            if ($cur_post['url'] != '')
                $user_contacts[] = _CHtml::link($lang_topic['Website'], _CHtml::encode($cur_post['url']));
        }        if ($_user['is_admmod'])
        {
            $user_info[] = '<dd>' . $lang_topic['IP'] . ': ' . _CHtml::link($cur_post['poster_ip'], array('forum/moderate', 'get_host' => $cur_post['id']));            if ($cur_post['admin_note'] != '')
                $user_info[] = '<dd>' . $lang_topic['Note'] . ': <strong>' . _CHtml::encode($cur_post['admin_note']) . '</strong>';
        }
    }
    ...
    else
    {
        ...
        $user_title = get_title($cur_post);        if ($_user['is_admmod'])
            $user_info[] = '<dd>' . $lang_topic['IP'] . ': ' . _CHtml::link($cur_post['poster_ip'], array('forum/moderate', 'get_host' => $cur_post['id']));        if ($_config['o_show_user_info'] == '1' && $cur_post['poster_email'] != '' && !$_user['is_guest'] && $_user['g_send_email'] == '1')
    ...
    {
        if (!$_user['is_guest'])
            $post_actions[] = '<li class="postreport">' . _CHtml::link($lang_topic['Report'], array('forum/misc', 'report' => $cur_post['id']));        if ($cur_topic['closed'] == '0')
        {
...
if ($_config['o_topic_views'] == '1')
    $db->setQuery('UPDATE forum_topics SET num_views=num_views+1 WHERE id=' . $id)->execute() or error('Unable to update topic', __FILE__, __LINE__, $db->error());$forum_id = $cur_topic['forum_id'];
