...
if (isset($_POST['form_sent']))
{
    ...
    while (list($key, $input) = @each($form))
    {
        ...
        if (array_key_exists('p_' . $key, $pun_config) && $pun_config['p_' . $key] != $input)
            $db->setQuery('UPDATE ' . $db->tablePrefix . 'config SET conf_value=' . $input . ' WHERE conf_name=\'p_' . $db->escape($key) . '\'')->execute() or error('Unable to update board config', __FILE__, __LINE__, $db->error());
    }
