...
function output_html($feed)
{
    ...
    foreach ($feed['items'] as $item)
    {
        ...
        echo '<li>' . CHtml::link($subject_truncated, array('forum/' . $item['link']), array('title' => pun_htmlspecialchars($item['title']))) . '</li>' . "\n";
    }
}
...
if ($action == 'feed')
{
    ...
    if (isset($_GET['tid']))
    {
        ...
        $feed = array(
            ...
            'link' => $pun_config['o_WEB_PATH'] . '/viewtopic.php?id=' . $tid,
        ...
        while ($cur_post = $db->fetch_assoc())
        {
            ...
            $item = array(
                ...
                'link' => $pun_config['o_WEB_PATH'] . '/viewtopic.php?pid=' . $cur_post['id'] . '#p' . $cur_post['id'],
            ...
            if ($cur_post['poster_id'] > 1)
            {
                ...
                $item['author']['uri'] = $pun_config['o_WEB_PATH'] . '/profile.php?id=' . $cur_post['poster_id'];
            }
    ...
    else
    {
        ...
        $feed = array(
            ...
            'link' => $pun_config['o_WEB_PATH'] . '/index.php',
        ...
        while ($cur_topic = $db->fetch_assoc())
        {
            ...
            $item = array(
                ...
                'link' => $pun_config['o_WEB_PATH'] . ($order_posted ? '/viewtopic.php?id=' . $cur_topic['id'] : '/viewtopic.php?id=' . $cur_topic['id'] . '&amp;action=new'),
            ...
            if ($cur_topic['poster_id'] > 1)
            {
                ...
                $item['author']['uri'] = $pun_config['o_WEB_PATH'] . '/profile.php?id=' . $cur_topic['poster_id'];
            }
...
else if ($action == 'online' || $action == 'online_full')
{
    ...
    while ($pun_user_online = $db->fetch_assoc())
    {
        ...
        {
            $users[] = ($pun_user['g_view_users'] == '1') ? CHtml::link(pun_htmlspecialchars($pun_user_online['ident']), array('forum/profile', 'id' => $pun_user_online['user_id'])) : pun_htmlspecialchars($pun_user_online['ident']);
            ++$num_users;
        }
        else
            ++$num_guests;
...
else if ($action == 'stats')
{
    ...
    echo $lang_index['Newest user'] . ': ' . (($pun_user['g_view_users'] == '1') ? CHtml::link(pun_htmlspecialchars($stats['last_user']['username']), array('forum/profile', 'id' => $stats['last_user']['id'])) : pun_htmlspecialchars($stats['last_user']['username'])) . '<br />' . "\n";
